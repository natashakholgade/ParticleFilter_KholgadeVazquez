...
\ rad = i\pi/180 \ \wedge\ map\_pos_i = [x + l_i\cos(rad),\ y + l_i\sin(rad)]\big)
\end{equation*}
...
other words,
\begin{equation*}
w = exp(sum(\mathcal{N}(L - \hat{L}, 0, \sigma_{beam})))
\end{equation*}
...
THe accompanying video show how our implementation worked. The
similarity between some places of the map difficultate convergence in
some cases, and parameters were adjusted to try to overcome these
situations.

% NOTE: Needs to be updated!
Note: to execute our accompanying code, use the function: 
$$runParticleFilter(map, dataLogName, desiredNumberParticles)$$


As the movies show, our particle filter algorithm can converge to the
correct global position is less than 200 iterations. For
\textit{robotdata1} we used 800 particles, which we depict as blue
dots. When the filter converges, we plot an oriented circle with
triangle depicting laser location, and use its pose to transform the
filtered laser log into red beams in world coordinates. When the
filter works properly, the laser readings align with map obstacles.
